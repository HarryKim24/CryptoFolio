...
try {
  execSync(`npx playwright test tests/home.spec.ts --project=chromium`, { stdio: 'inherit' });
} catch {
...
execSync(`${ffmpegPath} -y -ss 0.3 -i "${videoOutput}" -vf "fps=30,scale=1280:-1:flags=lanczos,palettegen" "${palettePath}"`, { stdio: 'inherit' });

console.log('🖼️ ffmpeg GIF 최적화 변환 중...');
execSync(`${ffmpegPath} -y -ss 0.3 -i "${videoOutput}" -i "${palettePath}" -lavfi "fps=30,scale=1280:-1:flags=lanczos [x]; [x][1:v] paletteuse" -loop 0 "${gifOutput}"`, { stdio: 'inherit' });

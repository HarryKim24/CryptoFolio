...
const videoOutput = path.join(outputDir, 'portfolio.mp4');
const gifOutput = path.join(outputDir, 'portfolio.gif');
const palettePath = path.join(outputDir, 'palette-portfolio.png');
...
async function main() {
  ...
  try {
    runCommand('npx playwright test tests/portfolio.spec.ts --project=chromium');
  } catch {
  ...
  const mp4Command = `${ffmpegPath} -y -ss 1 -i "${videoInput}" -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p "${videoOutput}"`;
  ...
  const paletteCommand = `${ffmpegPath} -y -ss 1 -i "${videoOutput}" -vf "fps=30,scale=960:-1:flags=lanczos,palettegen" "${palettePath}"`;
  runCommand(paletteCommand);

  console.log('Converting to GIF...');
  const gifCommand = `${ffmpegPath} -y -ss 1 -i "${videoOutput}" -i "${palettePath}" -lavfi "fps=30,scale=960:-1:flags=lanczos [x]; [x][1:v] paletteuse" -loop 0 "${gifOutput}"`;

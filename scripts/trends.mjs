...
try {
  execSync(`npx playwright test tests/trends.spec.ts --project=chromium`, { stdio: 'inherit' });
} catch {
...
execSync(`${ffmpegPath} -y -ss 0.2 -i "${videoInput}" -c:v libx264 -crf 18 -preset slow -pix_fmt yuv420p "${videoOutput}"`, { stdio: 'inherit' });
...
execSync(`${ffmpegPath} -y -ss 0.2 -i "${videoOutput}" -vf "fps=30,scale=960:-1:flags=lanczos,palettegen" "${palettePath}"`, { stdio: 'inherit' });

console.log('🖼️ ffmpeg GIF 최적화 변환 중...');
execSync(`${ffmpegPath} -y -ss 0.2 -i "${videoOutput}" -i "${palettePath}" -lavfi "fps=30,scale=960:-1:flags=lanczos [x]; [x][1:v] paletteuse" -loop 0 "${gifOutput}"`, { stdio: 'inherit' });

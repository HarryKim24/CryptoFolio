...
const videoOutput = path.join(outputDir, 'settings.mp4');
const gifOutput = path.join(outputDir, 'settings.gif');
const palettePath = path.join(outputDir, 'palette-settings.png');
...
async function main() {
  ...
  try {
    runCommand('npx playwright test tests/settings.spec.ts --project=chromium');
  } catch {
